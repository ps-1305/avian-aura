...
int min_Distance(double dist[], bool sptSet[]){
	double min = INF;
	int min_index;
...
double dijkstrasAlgorithm_RAIL(double graph[VERTICES][VERTICES],int src, int dest){
...
void dijsktra_rail(GtkWidget *widget, GdkEvent* event){
    ...
    int ecoPrice = (int) dijkstrasAlgorithm_RAIL(bus, parsedFrom, parsedTo);
    ...
    fprintf(rail_ticket, "Price : Rupees %d /-\n", ecoPrice);
    fclose(rail_ticket);
}
